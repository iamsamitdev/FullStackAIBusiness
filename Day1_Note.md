...
0. [Section 0: Setup Tools and Resources](#section-0-setup-tools-and-resources)
    - ติดตั้งเครื่องมือที่จำเป็น (VS Code, Node.js, PostgreSQL)
...
##### 1.5.1 เตรียม Assets ใส่ในโฟลเดอร์ `public/`
- โลโก้บริษัท
- รูปภาพประกอบ (Hero Image, Features Images)
- ไอคอนต่างๆ

##### 1.5.2 สร้าง Components สำหรับ landing Page ที่โฟลเดอร์ `components/landing/`
- Navbar.tsx
- Hero.tsx
- Features.tsx
- About.tsx
...
##### 1.5.3 โค้ดใน `components/landing/Navbar.tsx`
```tsx
"use client"
...
##### 1.5.4 โค้ดใน `components/landing/Hero.tsx`
```tsx
import Image from "next/image"
...
##### 1.5.5 โค้ดใน `components/landing/Features.tsx`
```tsx
function Features() {
...
##### 1.5.6 โค้ดใน `components/landing/About.tsx`
```tsx
"use client"
...
##### 1.5.7 โค้ดใน `components/landing/Team.tsx`
```tsx
import Image from "next/image"
import Link from "next/link"
...
##### 1.5.8 โค้ดใน `components/landing/Testimonial.tsx`
```tsx
"use client"
...
##### 1.5.9 โค้ดใน `components/landing/Blog.tsx`
```tsx
import Image from "next/image"
import Link from "next/link"
...
##### 1.5.10 โค้ดใน `components/landing/CTA.tsx`
```tsx
import Link from "next/link"
...
##### 1.5.11 โค้ดใน `components/landing/Footer.tsx`
```tsx
import Image from "next/image"
import Link from "next/link"
...
##### 1.5.12 โค้ดใน `components/landing/ScrollToTop.tsx`
```tsx
"use client"
...
##### 1.5.13 แก้ไข `app/(landing)/page.tsx`
```tsx
...
##### 1.5.14 `app/(landing)/globals.css`
...
##### 1.5.15 แก้ไข `app/(landing)/layout.tsx`
...
สร้างฐานข้อมูลใหม่สำหรับโปรเจกต์นี้ เช่น `ai_business_blog`
...
# สร้างฐานข้อมูลใหม่
CREATE DATABASE ai_business_blog;
...
#### 2.3 การติดตั้ง Payload CMS ลงในโปรเจ็กต์ Next.js
...
#### 2.6 สร้าง Collection Users และ Media
...
#### 2.7 สร้างไฟล์ Config ของ Payload
...
#### 2.13 อัปเดต `payload.config.ts` เพื่อเพิ่ม Collection Posts
```ts
...
##### 2.14 สร้าง Blog Layout และ Page
